...
app = FastAPI(
    title=settings.PROJECT_NAME if hasattr(settings, 'PROJECT_NAME') else "Smart Maintenance SaaS API",
...
async def health_check_db(db: AsyncSession = Depends(get_async_db)):
    try:
        ...
        if result.scalar_one() == 1:
            ...
            raise HTTPException(status_code=503, detail="Database connectivity check failed: Unexpected result.")
    except Exception as e:
