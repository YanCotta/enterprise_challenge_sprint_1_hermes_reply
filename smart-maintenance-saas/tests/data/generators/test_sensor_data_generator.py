...
from data.generators.sensor_data_generator import (
    SensorDataGenerator,
    SensorReading,
    SensorType,
)
...
def test_generate_normal_reading_temperature():
    generator = SensorDataGenerator(
        sensor_id="temp_sensor_1", sensor_type=SensorType.TEMPERATURE
    )
    ...
    assert (
        reading.metadata["generation_mode"] == "normal"
    )  # Changed metadata to sensor_metadata
    assert isinstance(reading.timestamp, datetime)


def test_generate_normal_reading_vibration():
    generator = SensorDataGenerator(
        sensor_id="vib_sensor_1", sensor_type=SensorType.VIBRATION
    )
    reading = generator.generate_reading()
    assert isinstance(reading, SensorReading)
...
def test_generate_normal_reading_pressure():
    generator = SensorDataGenerator(
        sensor_id="pressure_sensor_1", sensor_type=SensorType.PRESSURE
    )
    reading = generator.generate_reading()
    assert isinstance(reading, SensorReading)
    ...
    assert (
        1000 < reading.value < 1020
    )  # Assuming baseline is 1012 +/- noise and variation
    assert 0.95 <= reading.quality <= 1.0
    assert (
        reading.metadata["generation_mode"] == "normal"
    )  # Changed metadata to sensor_metadata
...
def test_generate_anomaly_spike():
    generator = SensorDataGenerator(
        sensor_id="temp_sensor_anomaly", sensor_type=SensorType.TEMPERATURE
    )
    ...
    assert (
        reading.metadata["generation_mode"] == "anomaly_spike"
    )  # Changed metadata to sensor_metadata


def test_generate_anomaly_drift():
    generator = SensorDataGenerator(
        sensor_id="temp_sensor_drift", sensor_type=SensorType.TEMPERATURE
    )
    ...
    assert abs(reading.value - generator.baseline["value"]) > (
        generator.baseline["value"] * generator.baseline["anomaly_factor_drift"] * 0.85
    )  # check if drift is significant
    assert 0.6 <= reading.quality <= 0.85
    assert (
        reading.metadata["generation_mode"] == "anomaly_drift"
    )  # Changed metadata to sensor_metadata


def test_generate_anomaly_stuck_at_value():
    generator = SensorDataGenerator(
        sensor_id="vib_sensor_stuck", sensor_type=SensorType.VIBRATION
    )
    ...
    assert (
        reading1.metadata["generation_mode"] == "anomaly_stuck_at_value"
    )  # Changed metadata to sensor_metadata
...
def test_generate_anomaly_stuck_at_zero():
    generator = SensorDataGenerator(
        sensor_id="pressure_sensor_zero", sensor_type=SensorType.PRESSURE
    )
    ...
    assert 0.6 <= reading.quality <= 0.85
    assert (
        reading.metadata["generation_mode"] == "anomaly_stuck_at_zero"
    )  # Changed metadata to sensor_metadata
...
def test_rounding():
    generator = SensorDataGenerator(
        sensor_id="test_round", sensor_type=SensorType.TEMPERATURE
    )
