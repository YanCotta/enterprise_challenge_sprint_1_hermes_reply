...
| `HumanDecisionRequiredEvent` | **NEW:** Human decision request from HumanInterfaceAgent for approval workflows | `payload` (DecisionRequest with request_id, decision_type, context, options, priority) |
| `HumanDecisionResponseEvent` | **NEW:** Human decision response from HumanInterfaceAgent with approval and justification | `payload` (DecisionResponse with request_id, decision, justification, operator_id, confidence) |
| `AgentStatusUpdateEvent` | Agent operational status reports *(future use)* | TBD |

### **NEW: Human Decision Event Structure**

**HumanDecisionRequiredEvent** enables human-in-the-loop decision points:

- **Decision Request Payload**: Complete DecisionRequest with request ID, decision type, and context
- **Decision Types**: Maintenance approval, emergency response, budget approval, schedule changes, quality inspection
- **Context Information**: Equipment details, priority levels, and relevant operational data
- **Options**: Available decision choices for the human operator
- **Correlation Support**: Full event traceability for decision audit trails

**HumanDecisionResponseEvent** delivers human decision results:

- **Decision Response Payload**: Complete DecisionResponse with chosen decision and justification
- **Operator Information**: Human operator identification and decision timestamp
- **Confidence Scoring**: Operator confidence level in the decision made
- **Justification**: Detailed reasoning behind the decision for audit and learning purposes
- **Correlation Tracking**: Links to original decision request events for complete workflow traceability

**Decision Workflow Benefits:**
- 🤝 **Human oversight** for critical maintenance decisions
- 📋 **Structured approvals** with consistent decision formats
- 🔍 **Audit trails** for compliance and process improvement
- ⚡ **Automated integration** with existing agent workflows
...
- **`agents/decision/reporting_agent.py`** - **NOVO: Analytics and reporting agent with chart generation and data visualization**
- **`agents/learning/learning_agent.py`** - **NOVO: RAG-based learning agent with ChromaDB and SentenceTransformers for knowledge management**
