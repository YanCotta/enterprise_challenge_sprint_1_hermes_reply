...
📸 **Screenshot 1.1: Docker Compose Startup**

![Docker Compose Startup](./screenshots/step1_1_docker_startup.png)

*Screenshot shows: Docker compose output with all services starting, including build process and container creation*
...
**Command Executed:**
```bash
curl -X GET "http://localhost:8000/health/db" -H "X-API-Key: your_default_api_key"
```

**Purpose**: Verify database connectivity and health.

**Expected Output**: JSON response confirming database is operational.

📸 **[SCREENSHOT PLACEHOLDER 1.4]**
*Note: Database health check included in Screenshot 1.2 above*
...
**Expected Output**: HTTP 201 response with sensor data ID.

📸 **Screenshot 3.1: Anomaly Detection Trigger**

![Anomaly Detection Trigger](./screenshots/step6_anomaly_trigger.png)

*Screenshot shows: Terminal with curl command execution and successful 201 response with sensor data ID*
...
### 3.2 Event Chain Processing
**Monitoring**: Live logs showing the complete event chain:
1. `SensorDataReceivedEvent` - Initial data ingestion
2. `DataProcessedEvent` - Data validation and processing
3. `AnomalyDetectedEvent` - Anomaly detection trigger
4. `AnomalyValidatedEvent` - Anomaly validation completion
5. Orchestrator notifications and predictions

**Purpose**: Demonstrate real-time event-driven architecture in action.

📸 **[SCREENSHOT PLACEHOLDER 3.2]**
*Note: Event chain processing captured in live log screenshots (2.1, 2.1b, 2.1c) above*
...
*Note: Report generation command and output was captured during the live demonstration. Please add your screenshot of the curl command and JSON response here*
...
### 5.2 Shutdown Verification
**Command Executed:**
```bash
docker compose ps
```

**Purpose**: Verify complete system cleanup.

**Expected Output**: Empty container list confirming all services stopped.

📸 **[SCREENSHOT PLACEHOLDER 5.2]**
*Note: Shutdown verification included in Screenshot 5.1 above showing empty container list*
