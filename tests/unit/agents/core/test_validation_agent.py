...
from smart_maintenance_saas.apps.agents.core.validation_agent import ValidationAgent
from smart_maintenance_saas.core.events.event_models import AnomalyDetectedEvent, AnomalyValidatedEvent
from smart_maintenance_saas.data.schemas import AnomalyAlert, SensorReading, ValidationStatus, AnomalyType
from smart_maintenance_saas.apps.rules.validation_rules import RuleEngine # Assuming RuleEngine can be mocked or instantiated simply
...
def mock_event_bus():
    bus = Mock()
    bus.publish = Mock(return_value=None) # Async mock if publish is async
    return bus
...
def mock_crud_sensor_reading():
    ...
    crud.get_sensor_readings_by_sensor_id = Mock(return_value=[]) # Async mock if it's async
    return crud
...
def mock_rule_engine():
    ...
    engine.evaluate_rules = Mock(return_value=(0.0, ["mock rule reason"])) # Async mock if it's async
    return engine
...
class TestValidationAgentProcessEnumUsage:
    ...
    async def test_process_publishes_correct_validation_status_enum_value(
        ...
        mock_rule_engine.evaluate_rules.return_value = (0.2, ["Rule: high confidence boost"]) # Boost confidence
        ...
        # If `get_sensor_readings_by_sensor_id` is async:
        mock_crud_sensor_reading.get_sensor_readings_by_sensor_id = Mock(return_value=[]) # Ensure it's an async compatible mock if needed
    ...
    async def test_process_false_positive_status(
        ...
        mock_rule_engine.evaluate_rules.return_value = (0.0, []) # No change from rules
        mock_crud_sensor_reading.get_sensor_readings_by_sensor_id.return_value = [] # No historical data
    ...
    async def test_process_further_investigation_status(
        ...
        # Arrange
        correlation_id = str(uuid.uuid4())
        anomaly_details_payload = {"sensor_id": "s3", "anomaly_type": "stuck", "severity": 2, "confidence": 0.5, "description": "mid conf"}
        triggering_data_payload = {"sensor_id": "s3", "value": 5.0, "timestamp": datetime.utcnow().isoformat(), "sensor_type": "vibration", "unit": "mm/s"}
        event = AnomalyDetectedEvent(anomaly_details=anomaly_details_payload, triggering_data=triggering_data_payload, correlation_id=correlation_id)

        mock_rule_engine.evaluate_rules.return_value = (0.0, [])
        mock_crud_sensor_reading.get_sensor_readings_by_sensor_id.return_value = []
