...
from smart_maintenance_saas.apps.agents.decision.scheduling_agent import SchedulingAgent, CalendarService
from smart_maintenance_saas.data.schemas import MaintenanceRequest, ScheduleStatus, OptimizedSchedule
from smart_maintenance_saas.core.events.event_models import MaintenancePredictedEvent
...
class TestSchedulingAgentFindBestQualifiedTechnician:
    ...
    def test_find_best_tech_all_skills_match(self, scheduling_agent):
        ...
        # Expected: tech_001 (John Smith) due to skill match and sorting (0.8 avail, 5 exp)
...
class TestSchedulingAgentFindAvailableSlot:
    ...
    @pytest.mark.freeze_time('2023-10-26 12:00:00') # Freeze time for consistent `datetime.utcnow()`
    def test_slot_found_and_booked(self, scheduling_agent, mock_calendar_service):
    ...
    @pytest.mark.freeze_time('2023-10-26 12:00:00')
    def test_no_slot_due_to_unavailability(self, scheduling_agent, mock_calendar_service):
        scheduling_agent.calendar_service = mock_calendar_service
    ...
    @pytest.mark.freeze_time('2023-10-26 12:00:00')
    def test_slot_found_but_booking_fails(self, scheduling_agent, mock_calendar_service):
        scheduling_agent.calendar_service = mock_calendar_service
        ...
        request = MaintenanceRequest(
            ...
            preferred_time_window_end=datetime(2023, 10, 27, 14, 0, 0)
        )

        slot = scheduling_agent._find_available_slot_for_technician(technician, request)

        assert slot is None
        expected_check_start = datetime(2023, 10, 27, 10, 0, 0)
        expected_check_end = expected_check_start + timedelta(hours=request.estimated_duration_hours)
        mock_calendar_service.check_availability.assert_called_with(technician["id"], expected_check_start, expected_check_end)
        mock_calendar_service.book_slot.assert_called_with(technician["id"], expected_check_start, expected_check_end, request.id)

    @pytest.mark.freeze_time('2023-10-28 10:00:00') # Saturday
    def test_slot_search_skips_weekend_and_finds_monday(self, scheduling_agent, mock_calendar_service):
        scheduling_agent.calendar_service = mock_calendar_service
        technician = MOCK_TECHNICIANS[0]
        request = MaintenanceRequest(
    ...
    @pytest.mark.freeze_time('2023-10-26 17:00:00') # End of day
    def test_slot_search_starts_next_day_if_too_late(self, scheduling_agent, mock_calendar_service):
        scheduling_agent.calendar_service = mock_calendar_service
        technician = MOCK_TECHNICIANS[0]
        request = MaintenanceRequest(
...
@patch('smart_maintenance_saas.apps.agents.decision.scheduling_agent.logging')
async def test_handle_event_logging_with_correlation_id(
    mock_logging,
    scheduling_agent # Use the fixture
):
    mock_logger = Mock()
    mock_logging.getLogger.return_value = mock_logger # Agent's self.logger will be this mock
    ...
    scheduling_agent.schedule_maintenance_task = Mock(
    ...
    await scheduling_agent.handle_maintenance_predicted_event("MaintenancePredictedEvent", event)
